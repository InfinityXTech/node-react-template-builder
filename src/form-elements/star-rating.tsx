...
export interface StarRatingProps
{
  ...
  caption?: string;
  ratingAmount?: number;
  onRatingClick?: (event: any, v: any) => void;
}
...
export default class StarRating extends React.Component<StarRatingProps, StarRatingState> {
  ...
  getStars() {
    let stars = '';
    const numRating = this.props.ratingAmount ?? 0;
  ...
  applyPrecision(val: any, precision: any) {
    return parseFloat(val.toFixed(precision));
  }

  getDecimalPlaces(num: any) {
    const match = (`${num}`).match(/(?:\.(\d+))?(?:[eE]([+-]?\d+))?$/);
    return !match ? 0 : Math.max(0, (match[1] ? match[1].length : 0) - (match[2] ? +match[2] : 0));
  }

  getWidthFromValue(val: any) {
    const { min } = this;
    const { max } = this;
  ...
  getValueFromPosition(pos: any) {
    const precision = this.getDecimalPlaces(this.props.step);
    const maxWidth = this.ratingContainer.offsetWidth;
    const diff = this.max - this.min;
    let factor = (diff * pos) / (maxWidth * (this.props.step ?? 0));
    factor = Math.ceil(factor);
    let val = this.applyPrecision(parseFloat(this.min + factor * (this.props.step ?? 0)), precision);
  ...
  getStarRatingPosition(val: any) {
    const width = `${this.getWidthFromValue(val)}%`;
    return width;
  }

  getRatingEvent(e: any) {
  ...
  updateRating(width: any, val: any) {
    this.setState({
      pos: width,
      rating: val,
    });
    ...
    return (
      <span className="react-star-rating">
        <span ref={c => this.rootNode = c} style={{ cursor: 'pointer' }} className={classes}>
          {starRating}
          <input type="hidden" name={this.props.name} value={this.state.ratingCache.rating} style={{ display: 'none !important', width: '65px' }} min={this.min} max={this.max} readOnly />
        </span>
      </span>
    );
  }
